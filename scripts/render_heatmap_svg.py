...
Reskinned with an observability-style heat scale (cool → hot) instead of
GitHub's green, and a header band that matches the other cards in this
profile (assets/trace-spans.svg).
...
# level 0..5, cool -> hot (mirrors the accent colors used in trace-spans.svg)
PALETTE = ["#161b22", "#0d3a5c", "#1f6feb", "#38BDF8", "#FFA33C", "#FF5C5C"]
BG = "#0d1117"
BORDER = "#30363d"
HEADER_BAND = "#161b22"
DIM = "#8b949e"
...
HEADER_H = 30       # title band, matches trace-spans.svg
TOP = HEADER_H + 30  # room for month labels below the header
STEP = 0.011         # diagonal stagger per (week+day)


def level_for(day: dict) -> int:
    lvl = day["level"]
    # stretch level 4 days into the hot top end when they're the best days
...
def main() -> None:
    ...
    parts = [
        ...
        f'<text x="20" y="{HEADER_H - 10}" font-size="15" font-weight="700" fill="{FG}">Activity</text>',
        f'<rect x="0" y="{HEADER_H}" width="{w}" height="1" fill="{BORDER}"/>',
    ]

    for week, label in month_marks:
        parts.append(
            f'<text x="{LEFT + week * (CELL + GAP)}" y="{HEADER_H + 24}" fill="{DIM}">{label}</text>'
        )
